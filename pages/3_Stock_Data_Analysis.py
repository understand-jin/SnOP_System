...
# 1️⃣ 기간별 위험 자재 요약 (탭)
# -----------------------------------------------------
st.subheader(f"🚨 {selected_month} 기간별 위험 자재 요약")
...
# -----------------------------------------------------
# 💾 가공된 데이터 최종 등록 (요청하신 부분)
# -----------------------------------------------------
if "stock_data" not in st.session_state:
    st.session_state["stock_data"] = {}

# 'datasets' 없이 바로 [월이름]을 키로 사용하여 가공된 DF 저장
st.session_state["stock_data"][selected_month] = {
    "df": final_df,
    "processed_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
}

st.info(f"✅ {selected_month} 가공 데이터가 'stock_data' 세션에 저장되었습니다.")
